...
class Config():
    ...
    def set_uclinux_dist(self, uclinux_dist):
        """Determines whether visibility of choice items should be
        compatible with the Kconfig version in uClinux-dist"""
        self.uclinux_dist = uclinux_dist
    ...
    def _get_visibility(self):
        if self.cached_visibility is None:
            ...
            if isinstance(self, Symbol) and self.is_choice_symbol_:
                if self.config.uclinux_dist:
                    vis = self.parent._get_visibility()
                else:

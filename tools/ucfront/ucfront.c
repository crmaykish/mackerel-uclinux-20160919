...
static void process_args(int argc, char **argv)
{
	...
	for (i=1; i<argc; i++) {
		...
		if (strncmp(argv[i], "-L", 2) == 0) {
			...
			for (k = 0; k < num_map_dirs; k++) {
				char *newval;
				int n = strlen(map_dirs[k].old);

				if (strncmp(map_dirs[k].old, e, n) == 0) {
					x_asprintf(&newval, "%s%s",
						map_dirs[k].new, &e[strlen(map_dirs[k].old)]);
					e = newval;
					break;
				}
				/* special case so that /lib/ will match /lib on the cmd line */
				if (n && map_dirs[k].old[n-1] == '/'
						&& strlen(e) == n - 1
						&& strncmp(map_dirs[k].old, e, n-1) == 0) {
					x_asprintf(&newval, "%s", map_dirs[k].new);
					e = newval;
					break;
				}
			}
	...
	if (mode == MODE_COMPILE) {
		...
		if (!nodefaultlibs) {
			...
			if (cplusplus) {
				tmp = find_gcc_file(compiler, "libgcc_s.so");
				if (tmp) {
					if (strstr(tmp, "sys-root") == NULL)
						libgcc = "-lgcc_s";
				}
			}

			tmp = find_gcc_file(compiler, "libgcc_eh.a");
			if (tmp)
				libgcc_eh = "-lgcc_eh";

			if (id_shared_library) {
				add_shared_lib(stripped_args, "-lc");
				add_shared_lib(stripped_args, libgcc);
				if (libgcc_eh)
					add_shared_lib(stripped_args, libgcc_eh);
			}
			else {
				/*
		...
		if (!nostartfiles && libtype != LIBTYPE_NONE) {
#if 0

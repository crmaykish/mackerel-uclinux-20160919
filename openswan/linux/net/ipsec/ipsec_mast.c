...
static inline int ipsec_mast_xmit2(struct sk_buff *skb)
{
	...
	return dst_output(dev_net(skb->dev), NULL, skb);
#elif defined(NETDEV_25)	/* 2.6 kernels */
...
#ifdef ipsec_alloc_netdev
	im = ipsec_alloc_netdev(sizeof(struct mastpriv), name, NET_NAME_UNKNOWN, ipsec_mast_netdev_setup);
#else

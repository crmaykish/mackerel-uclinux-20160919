...
static struct proto key_proto = {
	.name	  = "KEY",
	.owner	  = THIS_MODULE,
	.obj_size = sizeof(struct key_opt),
...
#ifdef NET_26
#if LINUX_VERSION_CODE >= KERNEL_VERSION(4,1,0)
	sk=(struct sock *)sk_alloc(net, PF_KEY, GFP_KERNEL, &key_proto, kern);
#elif defined(NET_26_24_SKALLOC)
	sk=(struct sock *)sk_alloc(net, PF_KEY, GFP_KERNEL, &key_proto);
#elif defined(NET_26_12_SKALLOC)
...
#ifdef CONFIG_PROC_FS
int
pfkey_show(struct seq_file *seq, void *offset)
{
	struct sock *sk;
...
int
pfkey_supported_show(struct seq_file *seq, void *offset)
{
	int satype;
...
int
pfkey_registered_show(struct seq_file *seq, void *offset)
{
	int satype;

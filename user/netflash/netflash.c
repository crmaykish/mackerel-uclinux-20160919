...
#define CMD_LINE_OPTIONS "abc:Cd:efFhiHjkKlL:no:pr:R:sStuv?" DECOMPRESS_OPTIONS HMACMD5_OPTIONS SETSRC_OPTIONS SHA256_OPTIONS UBOOT_OPTIONS
...
static int get_bootpart(void)
{
	...
	fp = popen(GRUBEDIT " " GRUBENVFILE " list", "r");
	if (fp) {
		for (;;) {
			line = NULL;

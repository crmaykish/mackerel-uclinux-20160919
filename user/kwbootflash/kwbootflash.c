...
/*
 * Determine if the current flash pages use the Linux default 1-bit hamming
 * ECC code. We look at the OOB data and see how big the ECC bytes are.
 * The Linux 1-bit hamming code only uses 24 bytes to store ECC. The Marvell
 * kirkwood reed-solomon ECC uses 40 bytes.
 */
int checkhamming(char *name)
{
	...
	for (i = 24; i < 40; i++) {
		if (buf[i] != 0xff) {
			trace("kwbootflash: looks like 4-bit capable ECC\n");
			return 0;
		}
	}

	trace("kwbootflash: looks like 1-bit hamming ECC\n");
	return 1;
}

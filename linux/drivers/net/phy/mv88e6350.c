 ...
 */
static int mv88e6350_probe(struct platform_device *pdev)
{
	struct device *dev = &pdev->dev;
	struct device_node *np = dev->of_node;
	struct device_node *mdio, *ethernet, *child;
	...
	for_each_available_child_of_node(np, child) {
		...
		addrp = of_get_property(child, "reg", NULL);
		if (!addrp)
			continue;

		addr = be32_to_cpup(addrp);
	}

	if (index < 0) {
		printk("mv88E6350: no switch defined?\n");
		return -ENODEV;
	}

	v = mv88e6350_read(mdio_bus, addr, 0x10, 3);

	mv88e6350_bus = mdio_bus;
	mv88e6350_mii = addr;
...
static int mv88e6350_remove(struct platform_device *pdev)
{
	struct device *dev = &pdev->dev;
...
static struct platform_driver mv88e6350_driver = {
...
{
        return platform_driver_register(&mv88e6350_driver);
}
module_init(mv88e6350_init_module);

static void __exit mv88e6350_cleanup_module(void)
{
        platform_driver_unregister(&mv88e6350_driver);
}

			    ...
			    struct mvneta_rx_desc *rx_desc)
{
	...
	case MVNETA_RXD_ERR_CRC:
		pp->dev->stats.rx_crc_errors++;
		netdev_err(pp->dev, "bad rx status %08x (crc error), size=%d\n",
			   status, rx_desc->data_size);
		break;
	case MVNETA_RXD_ERR_OVERRUN:
		pp->dev->stats.rx_fifo_errors++;
		netdev_err(pp->dev, "bad rx status %08x (overrun error), size=%d\n",
			   status, rx_desc->data_size);
		break;
	case MVNETA_RXD_ERR_LEN:
		pp->dev->stats.rx_length_errors++;
...
static void mvneta_rx_csum(struct mvneta_port *pp, u32 status,
			   struct sk_buff *skb)
{
	/* Work around for possible 10Mbit always CRC error problem */
	if (pp->speed == SPEED_10) {
...
static inline int mvneta_rx_check_error(struct mvneta_port *pp, struct mvneta_rx_desc *rx_desc)
{
	u32 status = rx_desc->status;
	...
	/*
	 * Ignore CRC errors when operating at 10Mbit. It seems likely
	 * there is a hardware bug, all 10Mbit packets have CRC errors.
	 * Continue and let upper layers do checksumming and find errors.
	 */
	if (pp->speed == SPEED_10) {
		if ((status & MVNETA_RXD_ERR_SUMMARY) == 0)
			printk("mvneta: 10Mb with no expected RX error?\n");
		if ((status & MVNETA_RXD_ERR_CODE_MASK) != MVNETA_RXD_ERR_CRC)
			return 1;
	} else if (status & MVNETA_RXD_ERR_SUMMARY) {
		return 1;
	}
		     ...
		     struct mvneta_rx_queue *rxq)
{
	...
	while (rx_done < rx_todo) {
		...
		if (mvneta_rx_check_error(pp, rx_desc)) {
		err_drop_frame:
			dev->stats.rx_errors++;
...
#ifdef CONFIG_LEDMAN
	ledman_cmdset((dev->name[3] == '1') ? LEDMAN_LAN2_TX :
		(dev->name[3] == '2') ? LEDMAN_LAN3_TX : LEDMAN_LAN1_TX);
#endif
...
#ifdef CONFIG_LEDMAN
	ledman_cmdset((napi->dev->name[3] == '1') ? LEDMAN_LAN2_RX :
		(napi->dev->name[3] == '2') ? LEDMAN_LAN3_RX : LEDMAN_LAN1_RX);
#endif

...
#ifdef CONFIG_MACH_6300_EX_DT
	/* Set the LED actions the way we want */
	gm_phy_write(hw, port, 22, 3);
	ledover = gm_phy_read(hw, port, 17);
	ledover = (ledover & 0xffc0) | 0x0015;
	gm_phy_write(hw, port, 17, ledover);
	gm_phy_write(hw, port, 16, 0x0101);
...
#ifdef CONFIG_MACH_6300_EX_DT
	/*
	 * We want the LED actions to remain intact. So re-write the values
	 * here. And don't let the port enter low power mode.
	 */
	gm_phy_write(hw, port, 22, 3);
	ctrl = gm_phy_read(hw, port, 17);
	ctrl = (ctrl & 0xffc0) | 0x0015;
	gm_phy_write(hw, port, 17, ctrl);
	gm_phy_write(hw, port, 16, 0x0101);
	gm_phy_write(hw, port, 22, 0);
#else

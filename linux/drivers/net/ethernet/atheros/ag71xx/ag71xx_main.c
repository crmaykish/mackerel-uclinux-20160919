...
#ifdef CONFIG_LEDMAN
	ledman_cmdset((ag->dev->name[3] == '0') ?  LEDMAN_LAN1_TX :
		LEDMAN_LAN2_TX);
#endif
	...
	case SIOCSMIIREG:
		if (ag->phy_dev == NULL) {
			if (ag71xx_get_pdata(ag)->switch_data)
				return ag71xx_ar7240_mii_ioctl(ag, ifr, cmd);
			break;
		}
...
#ifdef CONFIG_LEDMAN
	ledman_cmdset((ag->dev->name[3] == '0') ?  LEDMAN_LAN1_RX :
		LEDMAN_LAN2_RX);
#endif
...
static int ag71xx_probe(struct platform_device *pdev)
{
	...
	err = request_irq(dev->irq, ag71xx_interrupt, 0, dev->name, dev);
	if (err) {

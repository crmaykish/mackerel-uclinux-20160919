...
static void sci_set_mctrl(struct uart_port *port, unsigned int mctrl)
{
	...
	switch (port->line) {
		case 1: /* port 1 only */
			...
			if ((serial_port_in(port, SCFCR) & SCFCR_MCE) == 0) {
				if (mctrl & TIOCM_RTS)
					serial_port_out(port, SCSPTR, serial_port_in(port, SCSPTR) & ~0x40);
				else
					serial_port_out(port, SCSPTR, serial_port_in(port, SCSPTR) | 0x40);
			}
			break;
		...
		case 1: /* port 1 only */
			{
				...
				if ((serial_port_in(port, SCFCR) & SCFCR_MCE) == 0) {
					if (serial_port_in(port, SCSPTR) & 0x0040)
						rc &= ~TIOCM_RTS;
					if (serial_port_in(port, SCSPTR) & 0x0010)
						rc &= ~TIOCM_CTS;
				}

...
#define USR2_DCDIN  	 (1<<5)	 /* DCD input state */
 ...
 */
#define MCTRL_TIMEOUT	(100*HZ/1000)
...
static unsigned int imx_get_mctrl(struct uart_port *port)
{
	...
	if (sport->have_dtrdcd) {
		if (readl(sport->port.membase + USR2) & USR2_DCDIN)
			tmp &= ~TIOCM_CAR;
	}
	if (sport->have_dcd_gpio) {
		if (gpio_get_value(sport->have_dcd_gpio))
			tmp &= ~TIOCM_CAR;
		else
			tmp |= TIOCM_CAR;
	}
...
static void imx_set_mctrl(struct uart_port *port, unsigned int mctrl)
{
	...
	if (sport->have_dtr_gpio)
		gpio_set_value(sport->have_dtr_gpio, (mctrl & TIOCM_DTR) ? 0 : 1);
...
/*
 * Check and set port for DTE mode. We may already be in that mode if set
 * by boot loader. Or we may be told by platform data or device tree to put
 * the port into DTE mode.
 */
static int imx_setup_mode(struct imx_port *sport)
{
	unsigned int val, temp;

	val = readl(sport->port.membase + UFCR);
	if (UFCR & UFCR_DCEDTE)
		sport->dte_mode = 1;
	if (sport->dte_mode) {
		/* Clear signal meanings, they are changing sense */
		temp = readl(sport->port.membase + UCR3);
...
static int imx_startup(struct uart_port *port)
{
	...
	if (sport->dte_mode) {
		temp = readl(sport->port.membase + UCR3);
		/* Different bit meanings when in DTE mode */
		temp &= ~(UCR3_DCD | UCR3_RI | UCR3_DTREN);
		writel(temp, sport->port.membase + UCR3);
	}
		...
		struct platform_device *pdev)
{
	...
	if (of_get_property(np, "fsl,uart-has-dtrdcd", NULL))
		sport->have_dtrdcd = 1;

	if (of_get_property(np, "fsl,uart-has-dtr-gpio", NULL)) {
		gpio = of_get_named_gpio(np, "fsl,uart-has-dtr-gpio", 0);
		if (gpio_is_valid(gpio))
			sport->have_dtr_gpio = gpio;
		gpio_request(gpio, "UART DTR");
		gpio_direction_output(gpio, 0);
	}

	if (of_get_property(np, "fsl,uart-has-dcd-gpio", NULL)) {
		gpio = of_get_named_gpio(np, "fsl,uart-has-dcd-gpio", 0);
		if (gpio_is_valid(gpio))
			sport->have_dcd_gpio = gpio;
		gpio_request(gpio, "UART DCD");
		...
		struct platform_device *pdev)
{
	...
	if (pdata->flags & IMXUART_DTE)
		sport->dte_mode = 1;

	if (pdata->flags & IMXUART_HAVE_DTRDCD)
		sport->have_dtrdcd = 1;

	if (pdata->flags & IMXUART_HAVE_DTR_GPIO) {
		if (gpio_is_valid(pdata->dtr_gpio)) {
			sport->have_dtr_gpio = pdata->dtr_gpio;
			gpio_request(pdata->dtr_gpio, "UART DTR");
			gpio_direction_output(pdata->dtr_gpio, 0);
		}
	}

	if (pdata->flags & IMXUART_HAVE_DCD_GPIO) {
		if (gpio_is_valid(pdata->dcd_gpio)) {
			sport->have_dcd_gpio = pdata->dcd_gpio;
			gpio_request(pdata->dcd_gpio, "UART DCD");
			gpio_direction_input(pdata->dcd_gpio);
		}
	}
}

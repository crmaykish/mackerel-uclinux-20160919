...
#ifdef CONFIG_LEDMAN
	ledman_cmdset(LEDMAN_COM1_TX);
#endif

	/*
	 *	It seems that sometimes we get into this interrupt processing
	 *	code - but there is no TX empty interrupt event	currently
	 *	posted. Not entirely sure why. But it is bad if we blindly go
	 *	off through here, pushing 16 more chars into a potentially
	 *	already full FIFO.
	 *
	 *	I seem to have fixed it by locking around the start/stop TX
	 *	functions, and clearing all TX interrupts before TX start.
	 *	(Clearing TX interrupts after stop seems lock up the
	 *	transmitter sometimes (it uses edge triggered interrupts).
	 *	There is a race on maintaining the enabled/disabled TX state,
	 *	but that alone doesn't seem to be the whole problem. The
	 *	clearing of TX interrupts on start is needed as well.
	 *
	 *	So for now this is commented out. But if we still see problems
	 *	in usage, we may need to turn this back on.
	 */
	if ((__raw_readl(KS8695_IRQ_VA + KS8695_INTST) & (1 << KS8695_IRQ_UART_TX)) == 0)
		return IRQ_NONE;
...
{
	while (!(UART_GET_LSR(port) & URLS_URTE))
		barrier();
...
static struct uart_driver ks8695_reg = {
	.owner			= THIS_MODULE,
	.driver_name		= SERIAL_KS8695_DRVNAME,
